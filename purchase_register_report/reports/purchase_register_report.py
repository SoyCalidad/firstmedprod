...
class PurchaseRegister(models.Model):
    ...
    amount_untaxed = fields.Float(string='Base imponible', digits=(16, 4), readonly=True)
    amount_tax = fields.Float(string='IGV', digits=(16, 4), readonly=True)
    amount_total = fields.Float(string='Importe Total', digits=(16, 4), readonly=True)
    l_name = fields.Char(string='Razón Social', readonly=True)
    vat = fields.Char(string='Documento de Identidad', readonly=True)
    catalog_06_id = fields.Many2one('einvoice.catalog.06', string='Tipo', readonly=True)
    ...
    def _where(self):
        return """
            WHERE
                (m.type = 'in_invoice' OR m.type = 'in_refund')
